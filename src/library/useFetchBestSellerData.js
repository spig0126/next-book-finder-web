...
export default function useFetchBestSellerData() {
    const [bestSellerData, setBestSellerData] = useState({
        fictionBooks: defaultData,
        nonfictionBooks: defaultData,
        youngAdultBooks: defaultData,
        adviceBooks: defaultData,
    });
    ...
    const getBestSellersData = async () => {
        ...
        try {
            const fictionRes = await axios.get(
                `https://api.nytimes.com/svc/books/v3/lists/current/hardcover-fiction.json?api-key=${process.env.NEXT_PUBLIC_NYT_API_KEY}`
            );
            const nonfictionRes = await axios.get(
                `https://api.nytimes.com/svc/books/v3/lists/current/hardcover-nonfiction.json?api-key=${process.env.NEXT_PUBLIC_NYT_API_KEY}`
            );
            const youngAdultRes = await axios.get(
                `https://api.nytimes.com/svc/books/v3/lists/current/young-adult-hardcover.json?api-key=${process.env.NEXT_PUBLIC_NYT_API_KEY}`
            );
            const adviceRes = await axios.get(
                `https://api.nytimes.com/svc/books/v3/lists/current/advice-how-to-and-miscellaneous.json?api-key=${process.env.NEXT_PUBLIC_NYT_API_KEY}`
            );

            setBestSellerData({
                fictionBooks: fictionRes.data.results.books,
                nonfictionBooks: nonfictionRes.data.results.books,
                youngAdultBooks: youngAdultRes.data.results.books,
                adviceBooks: adviceRes.data.results.books,
            });
            console.log(
                fictionRes.data.results.books,
                nonfictionRes.data.results.books,
                youngAdultRes.data.results.books,
                adviceRes.data.results.books
            );
            setIsLoading(false);
        } catch (error) {
            setIsError(true);
    ...
    return { bestSellerData, isLoading, isError };
}
